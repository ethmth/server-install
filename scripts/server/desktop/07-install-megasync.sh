...
# Add the repository to Apt sources:
echo "deb [signed-by=/usr/share/keyrings/meganz-archive-keyring.gpg] https://mega.nz/linux/repo/Debian_13/ ./" | tee /etc/apt/sources.list.d/megasync.list
# Update and install
apt update
apt install megasync thunar-megasync megacmd


echo "To login to Megasync, you will probably have to use the Awesome WM (can start with startx)"
